...
**Status:** Complete (all 5 rules implemented and unit-tested)
...
- [x] **Weight plateau on a cut:** if 14-day rolling average weight change is within ±0.1% bodyweight/week AND goal is "cut" → suggest a calorie reduction (e.g. -100 to -200 kcal/day) or a diet break, with the reasoning shown.
- [x] **Under-eating protein:** if 7-day average protein intake < 90% of target → flag it with specific foods/servings needed to close the gap.
- [x] **Strength stall:** if top-set weight for a lift hasn't increased in N consecutive sessions (e.g. 4) at the same rep range → suggest a deload week or an accessory-volume change.
- [x] **Missed logging streak:** if no food log or no weigh-in for X days → surface a gentle "you've gone quiet" nudge.
- [x] **Goal weight reached:** when current weight crosses `goalWeightKg` → surface an insight prompting a decision: set a new goal weight, switch to maintenance, or hold this weight and shift into a recomposition phase.
